...
public class GraphitiDriver {
    ...
    public class DataProcessingThread extends Thread {
        ...
        private final byte[] KEY_EVENT_PATTERN = {0x1B, 0x32};
        private final byte[] TOUCH_EVENT_PATTERN = {0x1B, 0x44};
        ...
        DataProcessingThread(SerialPort comPort, BlockingQueue<byte[]> queue) {
        ...
        public void run() {
            ...
            while (running) {
                try {
                    length = comPort.readBytes(buffer, buffer.length);
                    // Processing buffer to find matching patterns
                    for (int i = 0; i < length - 1; i++) {
                        // Check for touch event pattern
                        if (buffer[i] == TOUCH_EVENT_PATTERN[0] && buffer[i + 1] == TOUCH_EVENT_PATTERN[1] && i + 6 < length) {
                            queue.put(Arrays.copyOfRange(buffer, i, i + 7));
                            i += 6;
                        }
                        // Check for key event pattern
                        else if (buffer[i] == KEY_EVENT_PATTERN[0] && buffer[i + 1] == KEY_EVENT_PATTERN[1] && i + 5 < length) {
                            queue.put(Arrays.copyOfRange(buffer, i, i + 6));
                            i += 5;
                        }
                    }
        ...
        public byte[] getLastTouchEvent() throws InterruptedException {
            for (byte[] bytes : queue) {
                if (bytes[0] == TOUCH_EVENT_PATTERN[0] && bytes[1] == TOUCH_EVENT_PATTERN[1]) {
                    queue.remove(bytes);
                    return bytes;
                }
            }
        ...
        public byte[] getLastKeyEvent() throws InterruptedException {
            for (byte[] bytes : queue) {
                if (bytes[0] == KEY_EVENT_PATTERN[0] && bytes[1] == KEY_EVENT_PATTERN[1]) {
                    queue.remove(bytes);
                    return bytes;
                }
            }
        ...
        public String getKeyEvent(byte[] keyEventBytes) {
            if (keyEventBytes == null || keyEventBytes.length < 4)
                return null;

            int keyValue = ((keyEventBytes[2] & 0xff) << 8) | (keyEventBytes[3] & 0xff);
            return keyMap.getOrDefault(keyValue, "Unknown Key");
        }
    ...
    public void setKeyEvent(boolean enable) throws IOException, InterruptedException {
        ...
        if (enable) {
            dataProcessingThread = new DataProcessingThread(this.comPort, queue);
            dataProcessingThread.start();
        } else {
            if (dataProcessingThread != null) {
                dataProcessingThread.stopRunning();
                dataProcessingThread.join();  // Ensure thread has terminated before setting to null
                dataProcessingThread = null;
            }
        }
    }

    public byte[] getLastTouchEvent() throws InterruptedException, IOException {
        byte[] touchEvent = dataProcessingThread.getLastTouchEvent();

        if (touchEvent != null) {
            return touchEvent;
        } else {
            throw new IOException("No data received from the touch event");
        }
    }
